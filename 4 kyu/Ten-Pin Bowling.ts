...
function bowlingScore(frames) {
    ...
    // Преобразуем фреймы в броски
    for (let i = 0; i < frameList.length; i++) {
        const frame = frameList[i];
        if (i < 9) { // Первые 9 фреймов
            if (frame === 'X') {
                rolls.push(10);
            } else if (frame.includes('/')) {
                rolls.push(parseInt(frame[0]));
                rolls.push(10 - parseInt(frame[0]));
            } else {
                rolls.push(parseInt(frame[0]));
                rolls.push(parseInt(frame[1]));
            }
        } else { // 10-й фрейм
            for (const c of frame) {
                if (c === 'X') rolls.push(10);
                else if (c === '/') rolls.push(10 - rolls[rolls.length - 1]);
                else rolls.push(parseInt(c));
            }
        }
    }
    ...
    // Подсчет очков для первых 9 фреймов
    for (let frame = 0; frame < 10; frame++) {
        if (frame < 9) {
            if (rolls[rollIndex] === 10) { // Страйк
                score += 10 + rolls[rollIndex + 1] + rolls[rollIndex + 2];
                rollIndex += 1;
            } else if (rolls[rollIndex] + rolls[rollIndex + 1] === 10) { // Спэр
            ...
            score += rolls[rollIndex] + rolls[rollIndex + 1] + (rolls[rollIndex + 2] || 0);
        }
    }
